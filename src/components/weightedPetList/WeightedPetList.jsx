...
export function weightedActivePetScore(data, pet, weightMap, includeXpFactor) {
  if (!pet?.petId || !data?.PetsCollection || !data?.PetsCollection[pet?.petId])
    return 0;
  ...
  return petDatum.BonusList.filter((bonus) => {
    ...
    .map((activePetBonus, i) => {
      ...
      if (includeXpFactor)
        weightedResult = weightedResult / petDatum.LevelXpRequired;
      console.log(
        "\tweightedscore",
        activePetBonus,
        bonusBase,
...
const WeightedPetList = ({ data, weightMap }) => {
  ...
  const renderPet = (petData) => {
    ...
    return (
      <PetItem
        ...
        weightMap={weightMap}
        petScoreFn={(pet) =>
          weightedActivePetScore(data, pet, weightMap, includeXpFactor)
        }
      />
    );
  };
  ...
  const sortedPets = petNameArray
    .map((petData) => {
      return {
        ...petData,
        weightedActivePetScore: weightedActivePetScore(
          data,
          petData,
          weightMap,
          includeXpFactor
  ...
  return (
    <Grid2 container spacing={1}>
      ...
      <Grid2 xs={12}>{sortedPets.map(renderPet)}</Grid2>
    </Grid2>
  );
};
